        ...
        <div class="search-header">
          ...
          <div class="suggestions" v-if="Array.isArray(suggestions) && suggestions.length > 0">
            <Suggestion
                ...
                :key="index"
                :title="card.name"
                @click="goToItemPage(category, card.id)"
            />
          </div>
        </div>
      </div>
...
const emit = defineEmits(["go-to-item"]);
...
const categories = [
  ...
  {label: "Кейс", value: "cases", icon: "fas fa-tshirt"},
...
const goToItemPage = (category, id) => {
  emit('go-to-item', category, id);
};
